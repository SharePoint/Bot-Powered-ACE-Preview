    ...
    if (model.indexSignature || model.children) {
        ...
        const types = model.children &&
            model.children.map((obj) => {
                return `${obj.name}${obj.flags.isOptional ? '?' : ''}: ${type.call(obj.signatures || obj.children ? obj : obj.type)} ${obj.defaultValue ? `= ${escape_1.escape(obj.defaultValue)}` : ''}`;
            });
        return `{ ${indexSignature ? indexSignature : ''}${types ? types.join('; ') : ''} }${model.defaultValue ? `= ${escape_1.escape(model.defaultValue)}` : ''}`;
    }
    return '{}';
}
...
function getStringLiteralType(model) {
    return `\\"${escape_1.escape(model.value)}\\"`;
}
